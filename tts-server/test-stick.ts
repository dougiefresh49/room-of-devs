 ...
 */
import { evaluateStickAxis, type StickArmState } from "./src/hid.js";
...
eq(
  evaluateStickAxis("armed", 64, "low"),
  { fire: false, state: "armed" },
  "low at threshold (strict <)",
);
...
eq(
  evaluateStickAxis("fired", 161, "low"),
  { fire: false, state: "fired" },
  "low above rearm stays fired",
);
...
eq(
  evaluateStickAxis("armed", 192, "high"),
  { fire: false, state: "armed" },
  "high at threshold (strict >)",
);
eq(evaluateStickAxis("armed", 193, "high"), { fire: true, state: "fired" }, "high crosses → fire");
eq(
  evaluateStickAxis("fired", 255, "high"),
  { fire: false, state: "fired" },
  "high held no re-fire",
);
eq(
  evaluateStickAxis("fired", 200, "high"),
  { fire: false, state: "fired" },
  "high still deflected",
);
...
eq(
  evaluateStickAxis("fired", 95, "high"),
  { fire: false, state: "fired" },
  "high below rearm stays fired",
);
