...
- **Mine T3 Code's UX for takeaways** (noted 2026-08-30): the owner rates T3's mobile and desktop apps as very polished and very easy to use (typing, WhisperFlow voice input, session start/reply flow). Before the framework build's UI rounds, spend a session cataloging what specifically makes it feel that way and which patterns the room's surfaces should steal.
...
- **The 10,000-ft view — planetary/starmap canvas** (noted 2026-07-29,
  ...
  auditioning: the dock, harbor, the settled system). Owner: "kool
  concept, not necessary for day 1 — revisit once the other pieces are
  in place."
...
- Product memory for Mikey (the "fell skateboarding, two weeks later my wrist hurts" recall from Theo's memory video). MemPalace (github.com/MemPalace/mempalace, MIT, v3.8.0, poked 2026-08-28) evaluated and skipped: Python + ChromaDB + local ONNX embeddings, MCP-only integration (no JSON API, CLI prints text), extraction layer is shaped for coding transcripts. The linkage it provides is embedding similarity over verbatim chunks, so the room's own path is: append-only JSONL of what the owner said, plus a small local embedding index (sqlite-vec + all-MiniLM or similar) queried at answer time. Revisit MemPalace only if plain BM25/grep over the log fails on paraphrase recall. Decision log row 10 tracks the open shape.
