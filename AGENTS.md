...
- Claim-at-start (#75, decisions row 29). Before starting tracked-worthy work, file a ticket if none exists, then claim it: swap its `state/*` label to `state/working` and drop a one-line "claimed, doing X" comment (`gh issue edit N --add-label state/working --remove-label state/open`, then `gh issue comment`). At settle, write conclusions back and set the closing state label. Exactly one `state/*` label per issue, always; `pnpm spine-lint` checks it and CI runs it. When reporting activity, cite the last substantive activity (last comment or commit touching the ticket), never `updatedAt`: that timestamp moves on label edits and is not progress.
