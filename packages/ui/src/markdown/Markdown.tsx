...
export function Markdown({ text, linkPolicy, className }: { text: string; linkPolicy: LinkPolicy; className?: string }) {
  const components: Components = {
    ...
    a: ({ children, href }) => linkPolicy === "inert" ? <span className="md-link" title={href}>{children}</span> : <a href={href} target="_blank" rel="noopener noreferrer">{children}</a>,
  };
  ...
  return <div className={className}><ReactMarkdown remarkPlugins={[remarkBreaks]} rehypePlugins={[rehypeSanitize]} components={components}>{text}</ReactMarkdown></div>;
}
